...
const int ghostMaxHits = 5; // Количество ударов для смерти скелета
...
int main() {
 ...
 while (!quit) {
    ...
    // Обновляем анимации второго Пакмана
    if (pacman2SpeedX != 0 || pacman2SpeedY != 0) { // Если Пакман движется
    ...
    } else { // Если Пакман в состоянии покоя
    ...
    // Если кнопка нажата, включаем скелетов
    if (isButtonPressed) {
    ...
    // Отрисовка игрового поля и кнопки
    draw_field();
    renderButton();
