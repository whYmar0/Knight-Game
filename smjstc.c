...
float ghostX = 14.0f, ghostY = 5.0f;  
...
int pacman2IdleFrameDelay = 225;
int buttonX = WIDTH / 2 - 1; // Центр экрана
...
const int ghostMaxHits = 1; // Количество ударов для смерти скелета
...
void renderGhost() {
    if (isGhostAlive) {
        SDL_Rect dstRectGhost = { (int)(ghostX * TEXTURE_SIZE), (int)(ghostY * TEXTURE_SIZE), TEXTURE_SIZE, TEXTURE_SIZE };
        if (isGhostTakingDamage) {
            // Рендерим анимацию урона
            SDL_RenderCopy(renderer, ghostDamageTextures[ghostDamageCurrentFrame], NULL, &dstRectGhost);
        } else {
            // Рендерим обычную анимацию
            SDL_RenderCopy(renderer, ghostRunTextures[ghostCurrentFrame], NULL, &dstRectGhost);
        }
    }
}
...
void draw_field() {
    ...
    // Рендер Pac-Man 2
    SDL_Rect dstRect2 = { (int)(pacman2X * TEXTURE_SIZE), (int)(pacman2Y * TEXTURE_SIZE), PACMAN_SIZE, PACMAN_SIZE };
...
void updateGhostDamageAnimation() {
    if (isGhostTakingDamage) {
        ...
        if (currentTime - ghostDamageLastFrameTime >= ghostDamageFrameDelay) {
            ghostDamageCurrentFrame++;
            ghostDamageLastFrameTime = currentTime;

            // Если кадры закончились, сбрасываем флаг анимации
            if (ghostDamageCurrentFrame >= ghostDamageFrameCount) {
                ghostDamageCurrentFrame = 0; // Сброс к первому кадру
                isGhostTakingDamage = 0;    // Завершаем анимацию урона
            }
        }
    }
}
...
void handleKeyDown2(SDL_Keycode key) {
    switch (key) {
        case SDLK_u: pacman2SpeedY = -0.1f; break;
        case SDLK_j: pacman2SpeedY = 0.1f; break;
        case SDLK_h: pacman2SpeedX = -0.1f; break;
        case SDLK_k: pacman2SpeedX = 0.1f; break;
    }
}
...
void handleKeyUp1(SDL_Keycode key) {
    switch (key) {
        case SDLK_w:
        case SDLK_s: pacman1SpeedY = 0.0f; break;
        case SDLK_a:
        case SDLK_d: pacman1SpeedX = 0.0f; break;
        case SDLK_c: // Нажатие C для удара
    if (!isPacmanAttacking) {
        isPacmanAttacking = 1;
        pacmanAttackCurrentFrame = 0;
        pacmanAttackLastFrameTime = SDL_GetTicks();

        // Проверяем попадание по скелету
        if (isGhostAlive && isNear(pacman1X, pacman1Y, ghostX, ghostY)) {
            ghostHitCount++;
            isGhostTakingDamage = 1; // Включаем анимацию урона
            ghostDamageCurrentFrame = 0; // Начинаем с первого кадра
            ghostDamageLastFrameTime = SDL_GetTicks();

            if (ghostHitCount >= ghostMaxHits) {
                isGhostAlive = 0; // Уничтожаем скелет
                ghostX = -1; // Убираем его с поля
                ghostY = -1;
            }
        }
    }
...
void handleKeyUp2(SDL_Keycode key) {
    switch (key) {
        case SDLK_u: 
        case SDLK_j: pacman2SpeedY = 0.0f; break;
        case SDLK_h: 
        case SDLK_k: pacman2SpeedX = 0.0f; break;
...
void renderButton() {
    SDL_Rect buttonRect = { buttonX * TEXTURE_SIZE, buttonY * TEXTURE_SIZE, 90, TEXTURE_SIZE };
...
void spawnGhosts() {
    for (int i = 0; i < MAX_GHOSTS; ++i) {
        ghosts[i].x = (rand() % (WIDTH - 2)) + 1; // Координаты внутри игрового поля
        ghosts[i].y = (rand() % (HEIGHT - 2)) + 1;
        ghosts[i].isAlive = 1;  // Скелет считается живым
        ghosts[i].hitCount = 0; // Счётчик ударов обнуляется
    }
}

// Проверка: уничтожены ли все скелеты
int areAllGhostsDead() {
    for (int i = 0; i < MAX_GHOSTS; ++i) {
        if (ghosts[i].isAlive) {
            return 0; // Найден хотя бы один живой скелет
        }
    }
    return 1; // Все скелеты уничтожены
}

// Логика урона по скелету
void handleGhostDamage(float pacmanX, float pacmanY) {
    for (int i = 0; i < MAX_GHOSTS; ++i) {
        if (ghosts[i].isAlive && isNear(pacmanX, pacmanY, ghosts[i].x, ghosts[i].y)) {
            ghosts[i].hitCount++;
            isGhostTakingDamage = 1; // Включаем анимацию урона
            ghostDamageCurrentFrame = 0; // Сброс анимации к первому кадру
            ghostDamageLastFrameTime = SDL_GetTicks();

            if (ghosts[i].hitCount >= ghostMaxHits) {
                ghosts[i].isAlive = 0; // Скелет умирает
            }
        }
    }
}
...
void renderGhosts() {
    for (int i = 0; i < MAX_GHOSTS; ++i) {
        if (ghosts[i].isAlive) {
            SDL_Rect dstRectGhost = {
                (int)(ghosts[i].x * TEXTURE_SIZE),
                (int)(ghosts[i].y * TEXTURE_SIZE),
                TEXTURE_SIZE,
                TEXTURE_SIZE
            };
            if (isGhostTakingDamage) {
...
while (!quit) {
    ...
    // Обновляем позиции Пакмана
    updatePacman1Position();
    updatePacman2Position();

    // Если все скелеты уничтожены, спавним новую партию
    if (isButtonPressed && areAllGhostsDead()) {
        spawnGhosts();
    }

    // Обновляем анимации первого Пакмана
    ...
    if (isPacmanAttacking) {
        if (currentTime - pacmanAttackLastFrameTime >= pacmanAttackFrameDelay) {
            pacmanAttackCurrentFrame++;
            pacmanAttackLastFrameTime = currentTime;

            if (pacmanAttackCurrentFrame >= pacmanAttackFrameCount) {
                pacmanAttackCurrentFrame = 0;
                isPacmanAttacking = 0; // Завершаем анимацию удара

                // Проверяем попадания по всем скелетам
                handleGhostDamage(pacman1X, pacman1Y);
            }
        }
    }

    // Обновляем анимацию урона скелетов
    updateGhostDamageAnimation();

    // Если кнопка нажата, включаем анимации скелетов
    if (isButtonPressed) {
        if (currentTime - ghostLastFrameTime >= ghostFrameDelay) {
            ghostCurrentFrame = (ghostCurrentFrame + 1) % ghostGifFrameCount;
            ghostLastFrameTime = currentTime;
        }
    }

    // Отрисовка игрового поля, скелетов и кнопки
    draw_field();
    renderGhosts();
    renderButton();
